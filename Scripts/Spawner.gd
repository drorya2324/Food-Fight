...
export var bystanfer_speed = 10
export var max_wait_time = 5

var bystanders = {}

func _enter_tree():
	bystanders = FileGrabber.get_files("res://Scenes/Bystanders/")
	randomize()
	set_timer()
...
func spawn_bystander():
	...
	bystander.set_linear_velocity($Forward.get_global_transform().basis[2].normalized() * bystanfer_speed)
